...
export default class AuthAppActions extends AuthAppGetters {
    ...
    async loginWithInexistentEmail() {
        await this.setEmail(USER.INVALID.EMAIL.FULL);
        await (await this.getEmailErrorField()).waitForDisplayed({ timeout: 3000 });
        await expect(await this.getEmailErrorField()).toHaveTextContaining(ErrorMessage.INEXISTENT_EMAIL);
    }
    ...
    async successfulOpenSignInPage() {
        await (await this.getTitleHeading()).waitForDisplayed({ timeout: 30000 });
