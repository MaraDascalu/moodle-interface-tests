...
import { CourseTitles, UriPath, WelcomeTitle, SemesterOptions, YearOptions, ProgressOptions, SortOptions } from './constants.js'
...
export default class CourseManagementActions extends CourseManagementGetters {
    ...
    async successfulNavigateToMainPage() {
        ...
        await expect (await this.getHeading()).toHaveText(WelcomeTitle);
    }
    ...
    async filterByYear() {
        await this.changeYearOption(YearOptions.PREVIOUS);
        await (await this.getCourseLink(1)).waitForDisplayed({ timeout: 10000 });
        ...
        await this.changeYearOption(YearOptions.CURRENT);
    ...
    async filterBySemester() {
        await this.changeSemesterOption(SemesterOptions.FIRST);
        await (await this.getCourseLink(1)).waitForDisplayed({ timeout: 10000 });
        ...
        await this.changeSemesterOption(SemesterOptions.ALL);
        await expect (await this.getCourseLink(1)).toHaveTextContaining(CourseTitles.ANNOUNCEMENTS);
    }
    ...
    async filterByProgress() {
        await this.changeProgressStatusOption(ProgressOptions.INPROGRESS);
        await (await this.getCourseLink(1)).waitForDisplayed({ timeout: 10000 });
        await expect (await this.getCourseLink(1)).toHaveTextContaining(CourseTitles.ANNOUNCEMENTS);
        await expect (await this.getCourseLink(2)).toHaveTextContaining(CourseTitles.CC);
        await this.changeProgressStatusOption(ProgressOptions.ALL);
        await expect (await this.getCourseLink(1)).toHaveTextContaining(CourseTitles.ANNOUNCEMENTS);
    }
    ...
    async sortCourses() {
        ...
        await this.changeSortCriteria(SortOptions.ACCESS);
        await (await this.getCourseLink(1)).waitForDisplayed({ timeout: 10000 });
        await expect (await this.getCourseLink(1)).toHaveTextContaining(CourseTitles.CC);
        await this.changeSortCriteria(SortOptions.TITLE);
    }
    ...
    async filterByMultipleCriteria() {
        ...
        await this.changeSemesterOption(SemesterOptions.FIRST);
        await expect (await this.getCourseLink(1)).toHaveTextContaining(CourseTitles.AA);
        await this.changeProgressStatusOption(ProgressOptions.PAST);
    ...
    async sortFilterCourses() {
        await this.changeSemesterOption(SemesterOptions.SECOND);
